...
const int WHAT_IS_GOOD_STEP = 1;
const int WHAT_IS_GOOD_INCREASE_EACH = 10;
const int WHAT_IS_GOOD_INIT = 6;
...
long long get_tw_from_str(const char* s) {

	std::string td_s(s);

	auto found = td_s.find("td");

	if (found == string::npos) {
		return 1000000000;
	}

	int num_start = found + 3;

	int num_end = num_start + 1;
	while (num_end < td_s.size() && td_s[num_end] != ' ') num_end++;
	...
	long long ans = stoi(td_s.substr(num_start, num_end - num_start));
...
const char* tw(std::istream& in) {
	if (best_decomposition != 0) {
		delete[] best_decomposition;
		best_decomposition = 0;
	}
	...
	auto abort_if_good = [&](int good_bags_num) -> bool {
		return get_tw_from_str(best_decomposition) < good_bags_num;
	};
	int what_is_good = WHAT_IS_GOOD_INIT;

	try{
		{
			auto g = load_pace_graph_impl(in);
...

		{
			try{
...

				{
					...
					for(int i=2;;++i){
						// std::cerr << "current dec tw" << get_tw_from_str(best_decomposition) << std::endl; 
						if (abort_if_good(what_is_good)) {
							return best_decomposition;
						}
						if (i % WHAT_IS_GOOD_INCREASE_EACH == 0) {
							what_is_good += WHAT_IS_GOOD_STEP;
						}
