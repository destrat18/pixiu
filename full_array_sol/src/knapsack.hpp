...
void dfs_topological_sort(int v, std::map<int, std::vector<int>>& g, std::set<int>& used, std::vector<int>& ans);
...
inline std::pair<std::vector<int>, int> solve_knapsack(const MyGraph& graph, const std::vector<wrap::TreeNode>& treedecomp) {
    ...
    std::vector<std::vector<std::vector<DptInfo>>> dpt;
    
    for (auto node: treedecomp) {
        std::vector<std::vector<DptInfo>> dpt_node;
        ...
        switch (node.node_type) {
                ...
                for (unsigned long long mask = 0; mask < ((unsigned long long)1 << node.bag.size()); mask++) {
                    ...
                    for (int capacity = 0; capacity < max_volume; capacity++) {
                        ...
                        if (!(bit_extractor_ull(introduced_vertex_idx) & mask)) { // introduced vertex not in the chosen set 
                            ...
                            // LASTFIX
                            if (!dpt[child_idx][mask_without_introduced_vertex][capacity].valid) {
                                dpt[i][mask][capacity].make_invalid();
                            } else {
                                dpt[i][mask][capacity].opt_cost = dpt[child_idx][mask_without_introduced_vertex][capacity].opt_cost;
                            }
                            ...
                            // LASTFIX
                            if (!dpt[child_idx][mask_without_introduced_vertex][capacity - vertex_volume].valid) {
                                dpt[i][mask][capacity].make_invalid();
                            } else {
                                dpt[i][mask][capacity].opt_cost = dpt[child_idx][mask_without_introduced_vertex][capacity - vertex_volume].opt_cost + vertex_cost;
                            }
            ...
            case wrap::FORGET: {
                ...
                for (unsigned long long mask = 0; mask < ((unsigned long long)1 << node.bag.size()); mask++) {
                    ...
                    for (int capacity = 0; capacity < max_volume; capacity++) {
                        if (dpt[child_idx][child_mask_with_forgotten_vertex][capacity].valid && dpt[child_idx][child_mask_without_forgotten_vertex][capacity].valid) {
                            int opt_if_take = dpt[child_idx][child_mask_with_forgotten_vertex][capacity].opt_cost;
                            int opt_if_not_take = dpt[child_idx][child_mask_without_forgotten_vertex][capacity].opt_cost;
                        ...
                        } else if (dpt[child_idx][child_mask_with_forgotten_vertex][capacity].valid) {
                            dpt[i][mask][capacity].opt_cost = dpt[child_idx][child_mask_with_forgotten_vertex][capacity].opt_cost;
                            dpt[i][mask][capacity].need_take = 1;
                        } else if (dpt[child_idx][child_mask_without_forgotten_vertex][capacity].valid) {
    ...
    for (int node_idx = curr_node_idx; node_idx >= 1; node_idx--) {
        ...
        switch (node.node_type) {
            ...
            case wrap::FORGET: {
                ...
                if (dpt[node_idx][current_mask][curr_capacity].need_take == 1) {
    ...
    // sort topologically wrt dependency edges

    std::map<int, std::vector<int>> dependency_graph;
    // add all taken vertices
    for (auto it: taken) {
        dependency_graph[it] = {};
    }
    // iterate over all edges in the graph and in case of dependency add edge to the graph
    for (auto [fr, to, edge]: graph.labeled_edges) {
        if (edge == 'd') {
            if (taken.find(fr) != taken.end() && taken.find(to) != taken.end()) {
                dependency_graph[to].push_back(fr);
            }
        }
    }
    ...
    for (auto it: taken) {
        if (used.find(it) == used.end()) {
            dfs_topological_sort(it, dependency_graph, used, topological_order);
        }
    }

    return {topological_order, opt};
...
void dfs_topological_sort(int v, std::map<int, std::vector<int>>& g, std::set<int>& used, std::vector<int>& ans) {
    ...
    for (auto it: g[v]) {
        if (used.find(it) == used.end()) {
            dfs_topological_sort(it, g, used, ans);
        }
    }
    ans.push_back(v);
}
