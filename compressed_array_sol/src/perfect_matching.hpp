...
template <class Num>
inline Num count_perfect_matchings(const MyGraph& graph, const std::vector<wrap::TreeNode>& treedecomp) {
...
template <class Num> 
inline Num count_perfect_matchings_brute_force(const MyGraph& graph) {
...
template <class Num> 
inline Num count_perfect_matchings_branching(MyGraph graph) {
    if (graph.vertices_non_zero_degree.empty()) {
        if (graph.alive_vertices.empty()) {
            // empty graph
            return 1;
        } else {
            // not all vertives are covered
            return 0;
        }
    }
    ...
    // (u, v) edge to branch
    int u = *graph.vertices_non_zero_degree.begin();

    for (int i = 0; i < graph.adg_set.size(); i++) {
        if (graph.adg_set[i].size() > graph.adg_set[u].size()) {
            u = i;
        }
    }
    ...
    g1.delete_edge(u, v);
    Num uv_not_in_matching = count_perfect_matchings_branching<Num>(g1);

    g2.delete_vertex(u);
    g2.delete_vertex(v);
    Num uv_in_matching = count_perfect_matchings_branching<Num>(g2);
